	...
	class Map {
    	...
    	private :
   		...
   		public :
			...
			int GetHitCondition( string userInput ) ;  // -1 : miss // 0, 1, 2, ... : ship number
...
int SBM::Map::GetHitCondition( string userInput ) {  // -1 : miss // 0, 1, 2, ... : ship number
	
	for ( int i = 0 ; i < blockLocation.size() ; i++ ) {
		if ( userInput == blockLocation[i] ) {
			return blockCondition[i] ;
		} // if
	} // for
